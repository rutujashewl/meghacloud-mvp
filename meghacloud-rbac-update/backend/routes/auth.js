...
router.post(
  ...
  async (req, res) => {
    ...
    // A self-registered user is the owner of a brand-new team/org — org_id points at themself.
    await db.prepare("UPDATE users SET org_id = ? WHERE id = ?").run(result.lastInsertRowid, result.lastInsertRowid);
