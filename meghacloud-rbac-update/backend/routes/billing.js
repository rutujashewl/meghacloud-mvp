...
router.use(blockAuditorWrites); // Auditor role: can view invoices, can't generate/pay them

const GST_RATE = 0.18;

async function orgInvoice(id, orgId) {
...
router.get("/invoices", async (req, res) => {
  const invoices = await db
    .prepare("SELECT * FROM invoices WHERE org_id = ? ORDER BY created_at DESC")
    .all(req.user.orgId);
  res.json({ invoices });
});
...
router.post("/invoices/generate", async (req, res) => {
  const servers = await db
    .prepare("SELECT * FROM servers WHERE org_id = ? AND status = 'running'")
    .all(req.user.orgId);
  ...
  const result = await db
    .prepare(
      `INSERT INTO invoices (user_id, org_id, invoice_number, subtotal, gst_amount, total_amount, status)
       VALUES (?, ?, ?, ?, ?, ?, 'pending')`
    )
    .run(req.user.id, req.user.orgId, invoice_number, subtotal, gst_amount, total_amount);
...
router.get("/invoices/:id", async (req, res) => {
  const invoice = await orgInvoice(req.params.id, req.user.orgId);
